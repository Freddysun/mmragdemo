...
class DocumentParser:
    ...
    def _ensure_index_exists(self, client: Optional[OpenSearch] = None) -> bool:
        ...
        if not client or not opensearch_config.index:
            return False
            
        if not client.indices.exists(index=opensearch_config.index):
    ...
    def _index_chunks_to_opensearch(self, chunks: List[str], source_key: str) -> bool:
        """将文本分块嵌入到OpenSearch中"""
        if not self.opensearch_client or not opensearch_config.index:
            return False
        ...
        try:
            ...
            for i, chunk in enumerate(chunks):
                try:
                    ...
                    doc = create_simplified_document(
                        content=chunk,
                        source=source_key,
                        chunk_id=chunk_id,
                        embedding=embedding,
    ...
    def process_pdf(self, file_key: str) -> Dict:
        ...
        try:
            ...
            if self.opensearch_client and opensearch_config.index:
